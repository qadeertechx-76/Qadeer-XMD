...
// Owner numbers
const ownerNumber = ["923369423531"];
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("QADEER-KING")) {
    console.log("❌ Invalid QADEER-KING session format");
    process.exit(1);
  }
  ...
  console.log("♻️ QADEER-KING session restored successfully");
}
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'QADEER-KING'}</p>
        </div>
      </body>
      </html>
    `);
...
async function connectToWA() {
  ...
  console.log(`🤖 QADEER-KING Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ QADEER-KING Bot connected to WhatsApp!");
      ...
      try {
        ...
        sock.sendMessage(ownerNumber[0] + '923369423531@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
